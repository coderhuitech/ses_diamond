$(function(){
	$('#show-jobs').live('click',function(){
		var request=$.ajax({
		  				type:'get',
		   				url: base_url+"/job_controller/show_jobs_in_phaseII_ajax",
						data: {},
						beforeSend:function(){},
	...
	$('.job-phseII-div').live('click',function(){
		...
		var request=$.ajax({
		  				type:'get',
		   				url: base_url+"/job_controller/select_job_by_job_id_for_phaseII_ajax",
						data: {job_id: job_id},
						beforeSend:function(){},
		   				success: function(data, textStatus, xhr) {
								//$('#result-div').html(data);
		...
		$.msgBox({
    			...
    			success: function (result) {
        			if (result == "Yes") {
						var request=$.ajax({
		  				type:'post',
		   				url: base_url+"/job_controller/save_phaseII_ajax",
	...
	$("#save_to_excel").live('click',function(e){
		var url='data:application/vnd.ms-excel,' + encodeURIComponent($('#all-karigar-balance-div > table:nth-child(1)').html()) 
			location.href=url
			return false
	});
	$('#show-jobs').trigger('click');
