$(function(){
	$('#show-jobs').live('click',function(){
		var request=$.ajax({
		  				type:'get',
		   				url: base_url+"/job_controller/show_jobs_for_pan_return_ajax",
						data: {},
						beforeSend:function(){},
	...
	$('.job-phseII-div').live('click',function(){
		...
		var request=$.ajax({
		  				type:'get',
		   				url: base_url+"/job_controller/select_job_by_job_id_for_pan_return_ajax",
						data: {job_id: job_id},
						beforeSend:function(){},
		   				success: function(data, textStatus, xhr) {
								//$('#result-div').html(data);
		...
		$.msgBox({
	    	...
	    	success: function (result) {
	        	if (result == "Yes") {
					...
					var request=$.ajax({
		  				type:'post',
		   				url: base_url+"/job_controller/save_pan_return_ajax",
		   				...
		   				success: function(data, textStatus, xhr) {
							...
							function useReturnData(data){
								...
								if(err!='error'){
									...
									$("#show-material-in-hand-karigar" ).html(response.filter('#report').html());
								}
							};//end of usereturndata
		   				}//end of success
					});//end of post request
