...
if ( ! function_exists('cell_format')){
	function cell_format($data,$type='number',$class='numeric',$id='ID',$prefix='',$suffix='',$colspan=0){
		...
		switch($type){
			case 'number':
					if($data<0){
						$cell=array('data'=>$prefix.number_format($data,2, '.', ','),'class' => $class.' negetive','id'=>$id);
					}else{
						$cell=array('data'=>$prefix.number_format($data,2, '.', ','),'class' => $class,'id'=>$id);
					}
			...
			case 'gold':
					if($data<0){
						$cell=array('data'=>$prefix.number_format($data,3, '.', ',').'<strong>&nbspg.</strong>'.$suffix,'class' => $class.' negetive','id'=>$id);

					}else{
						$cell=array('data'=>$prefix.number_format($data,3, '.', ',').'<strong>&nbspg</strong>'.$suffix,'class' => 'numeric '.$class,'id'=>$id);
...
if ( ! function_exists('create_log')){
	function create_log($msg="no error",$model="no model",$method="no funnction",$file_name="log_file.csv"){
		// open file
	   $fd = fopen($file_name, "a");
	   // append date/time to message
	   $sl=random_string('alnum',10);
	   $str =date("Y-m-d h:i:s", mktime()) .','.$sl.','.$model.','.$method.','. $msg ; 
	   // write string
...
if ( ! function_exists('base_url_with_port')){
	function base_url_with_port(){
		$actual_base_address='';
		/*list($a,$project_name,$c)=explode('/',$_SERVER['SCRIPT_NAME']);
		if($_SERVER['SERVER_PORT']!=80){
			$actual_base_address='http://'.$_SERVER['REMOTE_ADDR'].':'.$_SERVER['SERVER_PORT'].'/'.$project_name.'/';
		}else{
			$actual_base_address='http://'.$_SERVER['REMOTE_ADDR'].'/'.$project_name.'/';
		}
