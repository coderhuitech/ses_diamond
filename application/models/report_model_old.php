...
class Report_model extends CI_Model {
	...
	function select_jobs_by_period($date_from,$date_to){
		$sql="select date_format(tr_time,'%D-%M-%Y %H:%i') as job_date,job_id,order_id, product_code, pieces as qty, product_size, expected_gold, tr_time, gold_send, dal_send, pan_send, nitrick_returned, product_wt, gold_returned,(select table_status.status_name from table_status where table_status.status_ID = job_master.status) as status
			from job_master where date(`tr_time`)>=? and date(`tr_time`)<=?";
		$result=$this->db->query($sql,array($date_from,$date_to));
		if($result->num_rows()>0){
			return $result;
		}else{
	...
	function select_material_valuation(){
		$sql="select db2.rm_id,rm_name,material,round(material*(rm_master.rm_gold/100),3) as pure from
				(select rm_id,round(sum(closing_balance),3) as material from(select rm_id,emp_id, closing_balance from material_to_employee_balance where emp_id not in(28)) as db1 group by rm_id
				) as db2
	...
	function select_cash_n_hand_to_employee(){
		...
		(select * from employees_cash_balance where emp_id not in(28)) as db1";
		$result=$this->db->query($sql);
		if($result->num_rows()>0){
			return $result->row();
		}else{
	...
	function select_job_valuation(){
		$sql="select sum(pieces) as qty,sum(pieces*price) as lc, round(sum(gold_send+pan_send-gold_returned-nitrick_returned)*.92,3) as gold from job_master where status in(5,6,7,8,51) and in_stock=0";
		$result=$this->db->query($sql);
		if($result->num_rows()>0){
			return $result->row();
		}else{
	...
	function material_withdrawn_by_employee($emp_id,$date_from,$date_to,$transaction_type_id){
